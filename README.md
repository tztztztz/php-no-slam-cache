...
Every key if is not numeric will be converted to number by crc32 function, **but it is recommended to use only numbers for keys with this cache method**! Otherwise, there can be a conflict, as for two different string keys, crc32 function can produce the same numbers.
